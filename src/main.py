import random  # Changed alias to avoid confusion
import numpy as np
import pandas as pd
...
for index, order in orders_df.iterrows():
    assigned_robot = random.choice(range(num_robots))  # Use random for robot assignment
...
for robot_id, assigned_orders in orders_to_robots.items():
    ...
    model = PPO("MlpPolicy", env, verbose=1)  # proximal policy optimization
